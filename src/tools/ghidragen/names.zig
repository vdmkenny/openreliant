...
const orders = starlancer.orders;
const maneuvers = starlancer.maneuvers;

pub fn write(w: *Io.Writer) Io.Writer.Error!void {
    try w.writeAll("# Generated by ghidragen names from src/formats/vm_opcodes.zig, vm_commands.zig, orders.zig, maneuvers.zig and dte.Opcode.\n");
    try handlers(w);
    try commands(w);
    try orderGroups(w);
    try orderRoutines(w);
