...
//! `ghidra/names/LANCER.EXE.tsv`: a row for each VM opcode handler, each command implementation
//! and each order routine, and one for each group of the order table.
...
pub fn write(w: *Io.Writer) Io.Writer.Error!void {
    try w.writeAll("# Generated by ghidragen names from src/formats/vm_opcodes.zig, vm_commands.zig, orders.zig and dte.Opcode.\n");
    try handlers(w);
    try commands(w);
    try orderGroups(w);
    try orderRoutines(w);
