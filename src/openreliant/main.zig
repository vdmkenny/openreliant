...
const Options = struct {
    ...
    /// Whether a muzzle's flash lights what stands round it.
    flash_lights: game.guns.flash.Lights = .cast,
...
fn run(io: Io, gpa: Allocator, arena: Allocator, options: Options) !void {
    ...
    const flashes: game.guns.flash.Looks = try .create(arena, &textures, options.flash_lights);
...
test Options {
    ...
    try std.testing.expectEqual(.none, retro.flash_lights);
    try std.testing.expectEqual(.cast, plain.flash_lights);
