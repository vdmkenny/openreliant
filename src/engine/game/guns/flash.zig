...
pub const Lights = enum {
    /// **Improvement:** a flash casts a point light of its flare's colour (`flareColour`) while it
    /// lasts, dimming and drawing in as the flare shrinks, so that each shot lights up the hull
    /// round the gun.
    cast,
...
/// What a flash is drawn with: every gun type's flare but one, and that one's sheet.
pub const Look = enum {
    ...
    /// The look of a flash of `gun_type`. `guns_init` builds a mesh for each type, all alike but
    /// this one's; the port builds the two.
    pub fn of(gun_type: guns.GunType) Look {
...
/// How large every flash is drawn: the Laser Cannon's bolt, twice as wide and as high, and half as
/// long (`muzzle_flash_mesh_build`).
const size: Vector = @as(Vector, stats.gun_types[guns.GunType.laser_cannon.number()].bolt) * Vector{ 2, 2, 0.5 };

/// A flare's material: drawn with the flash's own texture coordinates, unlit, and added to what
/// stands behind it.
const material: srapiext.Material = .onePass(.{ .coordinates = .generated, .lit = false, .blend = .add });

/// The object flags of a flash's mesh (`muzzle_flash_create`): never culled, with its own texture
/// coordinates.
const flags: srapiext.ObjectFlags = .{ .not_culled = true, .own_first = true };

/// How far a flash's light reaches at its brightest.
const light_reach: f32 = 1500;
...
pub const Looks = struct {
    ...
    /// Builds both (`guns_init`): each a plume (`environfx.plumeMesh`) of `size`.
    pub fn create(gpa: Allocator, textures: *srtexture.Table, lights: Lights) (Allocator.Error || matmanager.Error)!Looks {
        var looks: Looks = .{ .meshes = undefined, .colours = undefined, .lights = lights };
        var made: usize = 0;
        ...
        for (std.enums.values(Look)) |look| {
            ...
            looks.meshes.set(look, try environfx.plumeMesh(gpa, size, material, nozzle, blades));
            made += 1;
            looks.colours.set(look, switch (look) {
                .flare => flareColour(&.{ .{ .image = nozzle }, .{ .image = blades } }),
...
pub const Flash = struct {
    ...
    pub fn init(flash: *Flash, looks: *const Looks, part: usize, attachment: *const shp.Attachment) void {
        ...
        const look: Look = .of(gun_type);
        flash.* = .{
            .part = part,
            .attachment = attachment,
            .look = look,
            .ticks = stats.flash_ticks[gun_type.number()],
            .level = .{.{ .mesh = looks.meshes.getPtrConst(look), .until = std.math.inf(f32) }},
            .object = .{ .flags = flags, .position = @splat(0), .radius = looks.meshes.getPtrConst(look).radius, .levels = &.{} },
            .light = switch (looks.lights) {
                .cast => .{ .mask = 0, .intensity = 0, .colour = looks.colours.get(look), .kind = .{ .point = .{ .position = @splat(0), .range = light_reach } } },
                .none => null,
            },
        };
    ...
    /// shot's type's ticks have passed.
    pub fn fire(flash: *Flash, kind: guns.GunType, now: i32) void {
        flash.until = now + stats.flash_ticks[kind.number()];
    }
...
pub const testing = struct {
    ...
    pub const Built = struct {
        ...
        pub fn init(gpa: Allocator, lights: Lights) !Built {
            // The texture cache keeps each file's name, without the directory the game names it by.
            const textures = try @import("../backdrop.zig").testing.Textures.initNames(gpa, &.{ "matflarea3", "matflareb3", "sfxalpha1" });
            errdefer textures.deinit(gpa);
            return .{ .textures = textures, .looks = try .create(gpa, &textures.table, lights) };
        }
...
test Looks {
    ...
    const built: testing.Built = try .init(gpa, .cast);
    defer built.deinit(gpa);
    const looks = &built.looks;

    // Both plumes are the Laser Cannon's bolt, twice as wide and half as long, and added unlit
    // with the flash's own coordinates.
    const flare = looks.meshes.getPtrConst(.flare);
    try std.testing.expectEqual(@as(Vector, .{ 60, 60, 600 }), flare.bounds[1]);
    try std.testing.expectEqual(material, flare.surfaces[1].material);
...
test Look {
    try std.testing.expectEqual(Look.sheet, Look.of(.gattling_plasma_cannon));
    try std.testing.expectEqual(Look.flare, Look.of(.laser_cannon));
...
test Flash {
    ...
    // Full size as it is lit, standing where the muzzle does on its part, its light as bright.
    flash.fire(.laser_cannon, 100);
    ...
    // A shot of a type that shows no flash lights it for no time, at no size.
    flash.fire(.turret_lasers, 200);
    try std.testing.expect(flash.show(200, carrier));
    try std.testing.expect(!flash.show(201, carrier));
}

test "a flash of a gun type that lasts no time shows at no size" {
    const gpa = std.testing.allocator;
    const built: testing.Built = try .init(gpa, .none);
    defer built.deinit(gpa);
    var attachment = std.mem.zeroes(shp.Attachment);
