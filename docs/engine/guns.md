...
- the beam strikes every object but the ship, the stand-ins and the disabled ones, whose bounding box it meets up to 40000 straight ahead. One listing no components takes the cannon's first damage times the ship's gun condition and the charge, on the quadrant the beam enters, its second damage over the first passing through, and its shields flare there unless it is cloaked. One listing components is struck part by part (`nova_strike_parts`, `0x0047B840`): each part whose mesh's box the beam meets takes the cannon's second damage times the charge against its component for each leaf of its collision tree the beam crosses, with a hit's burst there ([Effects](effects.md#sparks));
