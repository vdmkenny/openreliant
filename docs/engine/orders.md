...
| `0x80` | `list_nearby` | While it runs, the ship lists the objects near it: up to ten with object flag `0x2` whose collision spheres, widened by a constant, overlap its own, at `GameObject` offsets `0x6B4` (the count) and `0x6B8`, and up to ten others for which `0x00401980`, which projects the two objects' motion, answers yes for a time of 50 updates and a margin of 2000 units, at `0x6E0` and `0x6E4`. |
...
An object with flag `0x40`, `0x800` or `0x10000000` takes no order. Otherwise the current order
gives way at once when there is none, when it has yet to start, or when the new order is one-shot.
A started order gives way to Explode, and to any order when its own priority is zero or the new
order's is higher, running its `exit` as it does. Pushing any other order on it is a fatal error,
"Cannot set ai %s on ship %s: Still %s".
...
5. Afterwards it clears the throttle and both burns while the object has flag `0x20000`, both burns
   when it has no afterburner fuel, and reverse thrust when its flags lack `0x80`.

It runs from two places:

- `orders_update` (`0x0040C8F0`) runs `object_orders` once a frame for every object that is not
  disabled (object flag `0x400`, which `DisableObject` sets), the player's ship included.
...
ships' combat stats hold 1 at `+0x28`, and not while the ship has object flag `0x80000`.
