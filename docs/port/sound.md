...
`openreliant` sets the sound up as `WinMain` does, with 10 voices, the volumes of `[Sound]` in `starlancer.ini`, `bank_stdsmp` and `smp3d.fat`, and runs the frame's sound once the camera is placed. The fades step then too, where `tick_timer` steps them on a timer of its own every five ticks, which comes to the same while frames come faster. The sandbox starts the player's engine with its ship and plays a piece of music from `music`, `New_Mission01.wav` unless `--music` names another or `none`; `--no-sound` runs silent ([Platform](platform.md#running)).
